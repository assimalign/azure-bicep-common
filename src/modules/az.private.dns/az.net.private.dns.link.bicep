...
resource azVirtualNetworkResource 'Microsoft.Network/virtualNetworks@2021-02-01' existing = {
 name: replace(replace('${privateDnsNameVirtualNetwork}', '@environment', environment), '@location', location)
}
...
resource azPrivateNsVirtualNetworkLinkDeployment 'Microsoft.Network/privateDnsZones/virtualNetworkLinks@2020-06-01' = {
  ...
  name: replace(replace('${privateDnsNameVirtualLinkName}', '@environment', environment), '@location', location)
  location: 'global'
  properties: {
