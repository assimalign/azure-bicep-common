...
resource azApimApiDeployment 'Microsoft.ApiManagement/service/apis@2021-12-01-preview' = {
  ...
  properties: {
    ...
    authenticationSettings: empty(apimGatewayApiAuthenticationConfigs) ? json('null') : apimGatewayApiAuthenticationConfigs.authenticationType == 'OpenIDConnect' ? {
      openid: {
...
resource azApimApiPoliciesDeployment 'Microsoft.ApiManagement/service/apis/policies@2020-12-01' = if (!empty(apimGatewayApiPolicy)) {
  name: 'policy'
...
module azApimApiOperationDeployment 'az.apim.api.operation.bicep' = [for operation in apimGatewayApiOperations: if (!empty(apimGatewayApiOperations)) {
  name: 'az-apim-operation-${guid(replace(replace('${!empty(apimGatewayApiOperations) ? operation.apimGatewayApiOperationName : 'no-az-apim-operation'}', '', ''), '', ''))}'
  params: {
    region: region
...
output apimGatewayApi object = azApimApiDeployment
