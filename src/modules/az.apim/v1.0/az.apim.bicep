...
var apimCustomDomains = [for domain in apimGatewayConfigs.customDomains ?? []: {
  ...
  keyVaultId: domain.domainCertificate.sourceType != 'KeyVault' ? json('null') : replace(replace(domain.domainCertificate.sourceReference, '@environment', environment), '@region', region)
...
resource azApimGatewayDeployment 'Microsoft.ApiManagement/service@2021-12-01-preview' = {
...
module azApimGatewayRoleAssignment '../../az.rbac/v1.0/az.rbac.role.assignment.bicep' = [for appRoleAssignment in apimGatewayMsiRoleAssignments: if (contains(apimGatewayConfigs, 'enableMsi') && apimGatewayConfigs.enableMsi == true && !empty(apimGatewayMsiRoleAssignments)) {
  name: 'az-apim-rbac-${guid('${apimGatewayName}-${appRoleAssignment.resourceRoleName}')}'
  ...
  params: {
    ...
    resourcePrincipalIdReceivingRole: azApimGatewayDeployment.identity.principalId
  }
}]

// Deploy Certificates, if any
module azApimGatewayCertificatesDeployment 'az.apim.certificate.bicep' = [for certificate in apimGatewayCertificates: if (!empty(apimGatewayCertificates)) {
  name: !empty(apimGatewayCertificates) ? 'az-apim-certs-${guid('${apimGatewayName}/${certificate.apimGatewayCertificateName}')}' : 'no-apim-certs-to-deploy'
  scope: resourceGroup()
  params: {
    region: region
    environment: environment
  ...
  dependsOn: [
    azApimGatewayRoleAssignment
  ]
}]

// Deploy Backends, if any
module azApimGatewayBackendsDeployment 'az.apim.backend.bicep' = [for backend in apimGatewayBackends: if (!empty(apimGatewayBackends)) {
  name: !empty(apimGatewayBackends) ? 'az-apim-backends-${guid('${apimGatewayName}/${backend.apimGatewayBackendName}')}' : 'no-apim-backends-to-deploy'
  scope: resourceGroup()
  params: {
    region: region
    environment: environment
  ...
  dependsOn: [
    azApimGatewayDeployment
  ]
}]

// Deploy Loggers, if any
module azApimGatewayLoggersDeployment 'az.apim.logger.bicep' = [for logger in apimGatewayLoggers: if (!empty(apimGatewayLoggers)) {
  name: !empty(apimGatewayLoggers) ? 'az-apim-loggers-${guid('${apimGatewayName}/${logger.apimGatewayLoggerResourceName}')}' : 'no-apim-loggers-to-deploy'
  scope: resourceGroup()
  params: {
    region: region
    environment: environment
...
// Deploy APIs, if any
module azApimGatewayApisDeployment 'az.apim.api.bicep' = [for api in apimGatewayApis: if (!empty(apimGatewayApis)) {
  name: !empty(apimGatewayApis) ? 'az-apim-apis-${guid('${apimGatewayName}/${api.apimGatewayApiName}')}' : 'no-apim-apis-to-deploy'
  scope: resourceGroup()
  params: {
    region: region
    environment: environment
...
output apimGateway object = azApimGatewayDeployment
