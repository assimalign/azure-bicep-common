...
module azAppConfigurationRoleAssignment 'az.rbac.role.assignment.app.configuration.bicep' = if (resourceTypeAssigningRole == 'Microsoft.AppConfiguration/ConfigurationStores') {
  name: 'az-appc-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
...
module azKeyVaultRoleAssignment 'az.rbac.role.assignment.key.vault.bicep' = if (resourceTypeAssigningRole == 'Microsoft.KeyVault/Vaults') {
  name: 'az-kv-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azEventGridRoleAssignment 'az.rbac.role.assignment.event.grid.bicep' = if (resourceTypeAssigningRole == 'Microsoft.EventGrid/Domains') {
  name: 'az-egd-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azStorageAccountRoleAssignment 'az.rbac.role.assignment.storage.bicep' = if (resourceTypeAssigningRole == 'Microsoft.Storage/StorageAccounts') {
  name: 'az-stg-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azStorageAccountBlobContainerRoleAssignment 'az.rbac.role.assignment.storage.blob.bicep' = if (resourceTypeAssigningRole == 'Microsoft.Storage/StorageAccounts/BlobServices/Containers') {
  name: 'az-stg-blob-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azStorageFileShareAccountRoleAssignment 'az.rbac.role.assignment.storage.fileshare.bicep' = if (resourceTypeAssigningRole == 'Microsoft.Storage/StorageAccounts/FileServices/Shares') {
  name: 'az-stg-fs-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azStorageAccountQueueRoleAssignment 'az.rbac.role.assignment.storage.queue.bicep' = if (resourceTypeAssigningRole == '"Microsoft.Storage/StorageAccounts/QueueServices/Queues') {
  name: 'az-stg-queue-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azStorageAccountTableRoleAssignment 'az.rbac.role.assignment.storage.table.bicep' = if (resourceTypeAssigningRole == 'Microsoft.Storage/StorageAccounts/TableServices/Table') {
  name: 'az-stg-table-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azServiceBusRoleAssignment 'az.rbac.role.assignment.service.bus.bicep' = if (resourceTypeAssigningRole == 'Microsoft.ServiceBus/Namespaces') {
  name: 'az-sbn-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azServiceBusQueueRoleAssignment 'az.rbac.role.assignment.service.bus.queue.bicep' = if (resourceTypeAssigningRole == 'Microsoft.ServiceBus/Namespaces/Queues') {
  name: 'az-sbn-queue-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azServiceBusTopicRoleAssignment 'az.rbac.role.assignment.service.bus.topic.bicep' = if (resourceTypeAssigningRole == 'Microsoft.ServiceBus/Namespaces/Topics') {
  name: 'az-sbn-topc-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azCosmosRoleAssignment 'az.rbac.role.assignment.cosmos.bicep' = if (resourceTypeAssigningRole == 'Microsoft.DocumentDB/DatabaseAccounts') {
  name: 'az-docdb-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azSqlServerRoleAssignment 'az.rbac.role.assignment.sql.server.bicep' = if (resourceTypeAssigningRole == 'Microsoft.Sql/Servers') {
  name: 'az-sql-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azSqlServerManagedRoleAssignment 'az.rbac.role.assignment.sql.server.managed.bicep' = if (resourceTypeAssigningRole == 'Microsoft.Sql/ManagedInstances') {
  name: 'az-managedsql-role-assign-${guid('${resourceRoleName}-${resourcePrincipalIdReceivingRole}')}'
  scope: ResourceGroup
  params: {
    environment: environment
    region: region
...
module azMediaServicesRoleAssignment 'az.rbac.role.assignment.media.services.bicep' = if (resourceTypeAssigningRole == 'Microsoft.Media/MediaServices') {
...
module azCognitiveServicesRoleAssignment 'az.rbac.role.assignment.cognitive.services.bicep' = if (resourceTypeAssigningRole == 'Microsoft.CognitiveServices/Accounts') {
