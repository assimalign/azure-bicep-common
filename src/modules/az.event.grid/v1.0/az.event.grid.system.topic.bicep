...
module azEventGridSystemTopicSubscriptionsDeployment 'az.event.grid.system.topic.subscription.bicep' = [for (subscription, index) in eventGridSystemTopicSubscriptions: if (!empty(subscription)) {
...
module azEventGridSystemTopicRoleAssignment '../../az.rbac/v1.0/az.rbac.role.assignment.bicep' = [for roleAssignment in eventGridSystemTopicMsiRoleAssignments: if (eventGridSystemTopicMsiEnabled == true && !empty(eventGridSystemTopicMsiRoleAssignments)) {
  name: 'az-egst-rbac-${guid('${azEventGridSystemTopicDeployment.name}-${roleAssignment.resourceRoleName}')}'
