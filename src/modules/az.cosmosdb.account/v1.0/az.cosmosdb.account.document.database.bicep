...
module azCosmosAccountDocumentDatabaseContainerDeployment 'az.cosmosdb.account.document.database.container.bicep' = [for container in cosmosDbAccountDatabaseContainers: if(!empty(container)) {
  name: !empty(cosmosDbAccountDatabaseContainers) ? toLower('az-docdb-container-${guid('${azCosmosAccountDocumentDatabaseDeployment.id}/${container.databaseContainer}')}') : 'no-dbdocument-containers-to-deploy'
  scope: resourceGroup()
  params: {
    region: region
