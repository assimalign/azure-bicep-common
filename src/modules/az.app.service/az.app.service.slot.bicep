...
resource azAppServicePlanResource 'Microsoft.Web/serverfarms@2021-01-01' existing = {
  name: replace(replace(appServiceSlotPlanName, '@environment', environment), '@location', location)
  scope: resourceGroup(replace(replace(appServiceSlotPlanResourceGroup, '@environment', environment), '@location', location))
}
...
resource azAppServiceStorageResource 'Microsoft.Storage/storageAccounts@2021-04-01' existing = {
  name: replace(replace(appServiceSlotStorageAccountName, '@environment', environment), '@location', location)
  scope: resourceGroup(replace(replace(appServiceSlotStorageAccountResourceGroup, '@environment', environment), '@location', location))
}
...
resource azAppServiceInsightsResource 'Microsoft.Insights/components@2020-02-02-preview' existing = {
  name: replace(replace(appServiceSlotInsightsName, '@environment', environment), '@location', location)
  scope: resourceGroup(replace(replace(appServiceSlotInsightsResourceGroup, '@environment', environment), '@location', location))
}
...
resource azAppServiceFunctionSlotDeployment 'Microsoft.Web/sites/slots@2021-01-01' = if (appServiceSlotType == 'functionapp' || appServiceSlotType == 'functionapp,linux') {
  name: appServiceSlotType == 'functionapp' || appServiceSlotType == 'functionapp,linux' ? replace(replace('${appServiceName}/${appServiceSlotName}', '@environment', environment), '@location', location) : 'no-function-app-slot-to-deploy'
...
module azAppServiceFunctionRoleAssignment '../az.rbac/az.rbac.role.assignment.bicep' = [for appSlotRoleAssignment in appServiceSlotMsiRoleAssignments: if (appServiceSlotMsiEnabled == true && !empty(appServiceSlotMsiRoleAssignments)) {
