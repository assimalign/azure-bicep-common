...
var audiences = [for audience in appServiceAuthIdentityProviderAudiences: replace(replace(audience, '@environment', environment), '@location', location)]

//  1. Get the existing app resource to add the host 
resource azAppServiceResource 'Microsoft.Web/sites@2021-01-15' existing = {
  name: replace(replace(appServiceName, '@environment', environment), '@location', location)
  scope: resourceGroup()
}

// 2. 
resource azAppServiceAuthSettingsDeployment 'Microsoft.Web/sites/config@2021-01-15' = {
  name: replace(replace('${appServiceName}/authsettingsV2', '@environment', environment), '@location', location)
  properties: {
    globalValidation: {
