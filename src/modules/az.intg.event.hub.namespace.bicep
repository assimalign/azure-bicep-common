...
resource azEventHubNamespaceDeployment 'Microsoft.EventHub/namespaces@2021-01-01-preview' = {
  name: replace('${eventHubNamespace}', '@environment', environment)
...
module azEventHubsDeployment './az.intg.event.hub.bicep' = [for (hub, index) in eventHubs: if (!empty(hub)) {
