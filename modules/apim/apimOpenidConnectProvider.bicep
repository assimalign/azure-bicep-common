...
var keyVaultName = contains(apimGatewayOpenIdConnectClientSecret, environment) ? 
  apimGatewayOpenIdConnectClientSecret[environment].keyVaultName : apimGatewayOpenIdConnectClientSecret.default.keyVaultName
...
var test = keyVault.getSecret(keyVaultSecretName)

resource t 'Microsoft.ApiManagement/service/openidConnectProviders@2021-12-01-preview' = {
  name: replace(replace(apimGatewayOpenIdConnectName, '@environment', environment), '@region', region)
  properties: {
    clientId: contains(apimGatewayOpenIdConnectClientId, environment) ? apimGatewayOpenIdConnectClientId[environment] : apimGatewayOpenIdConnectClientId.default
    clientSecret: keyVault.getSecret(keyVaultSecretName)
...
//     apimGatewayOpenIdConnectClientSecret: keyVault.getSecret(keyVaultSecretName)
